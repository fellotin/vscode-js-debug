...
export class Binder implements IDisposable {
	...
	private reportBootTelemetry(rawParams: AnyLaunchConfiguration) {
		...
		} as AnyResolvingConfiguration) as unknown as {
			[key: string]: unknown;
		};
	...
	private async captureLaunch(
		...
		try {
			...
			this._rootServices
				.get<ILogger>(ILogger)
				.warn(LogTag.RuntimeLaunch, "Launch returned error", {
					error: e,
...
export const isTargetTreeNode = (node: TreeNode): node is TargetTreeNode =>
	!!node.value;
