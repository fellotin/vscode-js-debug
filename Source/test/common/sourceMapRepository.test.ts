...
describe("ISourceMapRepository", () => {
	[
		{
			name: "TurboSearchStrategy",
			create: () => new TurboSearchStrategy(Logger.null),
		},
	].forEach((tcase) =>
		describe(tcase.name, () => {
			...
			const gatherFileList = (
				rootPath: string,
				firstIncludeSegment: string
			) =>
				new FileGlobList({
					rootPath,
					patterns: [
			...
			const gatherSm = async (list: FileGlobList) => {
				...
				>({
					...
					processMap: async (m) => {
						...
						expect(cacheKey).to.be.within(
							Date.now() - 60 * 1000,
							Date.now() + 1000
						);
						rest.compiledPath = fixDriveLetterAndSlashes(
							rest.compiledPath
						);
						return rest;
					},
				...
				return result.values.sort(
					(a, b) => a.compiledPath.length - b.compiledPath.length
				);
			};
			const gatherSmNames = async (list: FileGlobList) => {
				const result = await gatherSm(list);
				return result.map((r) =>
					fixDriveLetterAndSlashes(r.compiledPath)
				);
			};

			const gatherAll = (list: FileGlobList) => {
				return r
					.streamAllChildren(list, (m) => m)
					.then((r) => r.sort());
			};

			it("no-ops for non-existent directories", async () => {
				expect(
					await gatherSm(gatherFileList(__dirname, "does-not-exist"))
				).to.be.empty;
			});
			...
			it("discovers source maps and applies negated globs", async () => {
				expect(
					await gatherSm(
						gatherFileList(workspaceFolder, testFixturesDirName)
					)
				).to.deep.equal([
					{
					...
					},
					{
						...
						sourceMapUrl: absolutePathToFileUrl(
							join(testFixturesDir, "nested", "d.js.map")
						),
					},
					{
						compiledPath: fixDriveLetter(
			...
			it("applies second patterns (vscode#168635)", async () => {
				...
				expect(
					...
					fixDriveLetter(
						join(testFixturesDir, "otherFolder", "f.js")
					),
				]);
			});
			...
			it("greps inside node_modules explicitly", async () => {
				expect(
					await gatherSm(
						gatherFileList(
							join(testFixturesDir, "node_modules"),
							"."
						)
					)
				).to.deep.equal([
