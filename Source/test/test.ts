...
export class TestP implements ITestHandle {
	...
	waitForSource(filter?: string): Promise<Dap.LoadedSourceEventParams> {
		return this.dap.once("loadedSource", (event) => {
			return (
				filter === undefined ||
				(event.source.path || "").indexOf(filter) !== -1
			);
		});
	}
	...
	async _init(
		...
		const { sessionId } = (await testSession.Target.attachToTarget({
			targetId:
				this._target instanceof BrowserTarget
...
export class TestRoot {
	...
	_browserLauncher:
		| BrowserLauncher<AnyChromiumLaunchConfiguration>
		| undefined;
	...
	async runScript(
		...
		await this.initialize;
		this._launchUrl = path.isAbsolute(filename)
			? filename
			: path.join(testFixturesDir, filename);
	...
	async runScriptAsRemote(
		...
		if (
			...
			tmpPath.startsWith("/var/folders")
		) {
	...
	async attachNode(
		...
		const result = await new Promise<ITestHandle>(
			(f) => (this._launchCallback = f)
		);
		await result.load();
		return result as NodeTestHandle;
	}
