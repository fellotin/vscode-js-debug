		...
		servers.map((server) => {
			return new Promise((resolve, reject) => {
				...
				server.once("close", (code) =>
					reject(new Error(`Exited with ${code}, stderr=${error}`))
				);
	...
	testFunction(test, async function () {
		...
		try {
			...
			await fn({
				golden,
				r: root,
				context: this as Mocha.Context & { test: Mocha.Runnable },
			});
		} finally {
			try {
...
itIntegratesBasic.only = (
	test: string,
	fn: (s: IIntegrationState) => Promise<void> | void
) => itIntegratesBasic(test, fn, it.only);
itIntegratesBasic.skip = (
	test: string,
	fn: (s: IIntegrationState) => Promise<void> | void
...
export async function waitForPause(
	p: ITestHandle,
	cb?: (threadId: number) => Promise<void>
) {
