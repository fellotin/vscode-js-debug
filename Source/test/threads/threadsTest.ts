...
describe("threads", () => {
	...
	describe("stepping", () => {
      ...
      `);
		...
		itIntegrates("cross thread", async ({ r }) => {
			...
			const { threadId: secondThreadId } = p.log(
				await worker.dap.once("stopped")
			);
		...
		itIntegrates(
			"cross thread constructor source map predicted",
			async ({ r }) => {
				...
				await p.dap.setBreakpoints({
					source: {
						path: p.workspacePath("web/does-not-exist.html"),
					},
	...
	describe("pause on exceptions", () => {
		...
		itIntegrates("cases", async ({ r }) => {
			...
			await p.evaluate(
				`setTimeout(() => { try { throw new Error('hello'); } catch (e) {}})`
			);
			...
			p.evaluate(
				`new Promise((res, rej) => rej(new Error('oh no!'))).catch(err => {})`
			);
			await waitForPauseOnException(p);
		...
		itIntegrates("pauses on conditional exceptions", async ({ r }) => {
			...
			await p.dap.setExceptionBreakpoints({
				...
				filterOptions: [
					{
						filterId: "all",
						condition: 'error.message.includes("bye")',
					},
				],
			});
			p.evaluate(
			...
			p.evaluate(
				`setTimeout(() => { try { throw new Error('goodbye'); } catch (e) {} })`
			);
			await waitForPauseOnException(p);
			...
			p.evaluate(`setTimeout(() => { throw new Error('hello'); })`);
			p.evaluate(
				`setTimeout(() => { try { throw new Error('goodbye1'); } catch (e) {} })`
			);
		...
		itIntegrates(
			"deals with syntax errors in conditional exception bps",
			async ({ r }) => {
				const p = await r.launchAndLoad("blank");
