...
async function setupCRA(
	...
	const setupProc = cp.spawn(
		"npx",
		["create-react-app", ...args, projectName],
		{
			cwd,
			stdio: "pipe",
			env: process.env,
	...
	setupProc.stderr.on("data", (d) =>
		console.error(d.toString().replace(/\r?\n$/, ""))
	);
...
async function startDevServer(
	...
	devServerProc.stdout.on("data", (d) => {
		...
		if (d.includes("You can now view")) {
			...
			devServerListening.reject(
				new Error("Failed to start the dev server: " + d)
			);
		}
