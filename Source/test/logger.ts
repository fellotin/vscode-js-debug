...
export class Logger {
	...
	public logVariable(
		...
		return walkVariables(
			...
			(variable, depth) => {
				...
				const namedCount = variable.namedVariables
					? ` named=${variable.namedVariables}`
					: "";
				...
				let suffix = options.logInternalInfo
					? `${type}${namedCount}${indexedCount}`
					: "";
	...
	async logOutput(params: Dap.OutputEventParams, options?: ILogOptions) {
		...
		if (params.variablesReference) {
			const result = await this._dap.variables({
