...
export class GoldenText {
	...
	constructor(
		...
		this._workspaceFolder =
			urlUtils.platformPathToPreferredCase(workspaceFolder);
	}
	...
	assertLog(
		...
		if (!fs.existsSync(goldenFilePath)) {
			...
			const expectations = trimLineWhitespace(
				fs.readFileSync(goldenFilePath).toString("utf-8")
			);
			...
			try {
				...
				fs.writeFileSync(goldenFilePath + ".actual", output, {
					encoding: "utf-8",
				});
				throw err;
			}
	...
	_sanitize(value: string): string {
		...
		const replacePath = (needle: string, replacement: string) => {
			...
			const safeStr = escapeRegexSpecialChars(
				forceForwardSlashes(needle),
				"/"
			);
			...
			value = value.replace(
				re,
				(_match, trailing) =>
					replacement + forceForwardSlashes(trailing)
			);
		};
	...
	_logObject(
		...
		const dumpValue = (
			...
			if (typeof value === "object" && value !== null) {
				...
				lines.push(
					prefixWithName + this._sanitize(value).replace(/\n/g, " ")
				);
			}
		};
