...
export async function run(): Promise<void> {
	...
	if (process.env.FRAMEWORK_TESTS) {
		await addFile(
			rel("framework/reactTest"),
			() => import("./framework/reactTest")
		);
	} else {
		await addFile(
			rel("testIntegrationUtils"),
			() => import("./testIntegrationUtils")
		);
		...
		await addFile(
			rel("browser/framesTest"),
			() => import("./browser/framesTest")
		);
		await addFile(
		...
		await addFile(
			rel("evaluate/evaluate"),
			() => import("./evaluate/evaluate")
		);
		await addFile(
			rel("sources/sourcesTest"),
			() => import("./sources/sourcesTest")
		);
		await addFile(
			rel("stacks/stacksTest"),
			() => import("./stacks/stacksTest")
		);
		await addFile(
			rel("threads/threadsTest"),
			() => import("./threads/threadsTest")
		);
		await addFile(
			rel("variables/variablesTest"),
			() => import("./variables/variablesTest")
		);
		await addFile(
			rel("console/consoleFormatTest"),
			() => import("./console/consoleFormatTest")
		);
		await addFile(
			rel("console/consoleAPITest"),
			() => import("./console/consoleAPITest")
		);
