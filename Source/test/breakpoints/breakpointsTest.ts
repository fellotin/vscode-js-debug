...
describe("breakpoints", () => {
	describe("configure", () => {
		...
		itIntegrates("absolute paths in source maps", async ({ r }) => {
			...
			createFileTree(cwd, {
				"pause.js": await readfile(
					r.workspacePath("web/browserify/pause.js")
				),
				"pause.html": await readfile(
					r.workspacePath("web/browserify/pause.html")
				),
				"pause.js.map": (
					await readfile(
						r.workspacePath("web/browserify/pause.js.map")
					)
				).replace(
					/"([a-z0-9]+.ts)"/g,
					`"${forceForwardSlashes(
						r.workspacePath("web/browserify")
					)}/$1"`
				),
			});
		...
		itIntegrates("source map that's path mapped", async ({ r }) => {
			...
			createFileTree(cwd, {
				"app.ts": await readfile(
					r.workspacePath("web/pathMapped/app.ts")
				),
				"app.js": (
					await readfile(r.workspacePath("web/pathMapped/app.js"))
				).replace("app.js.map", "mappedPath/app.js.map"),
				"index.html": await readfile(
					r.workspacePath("web/pathMapped/index.html")
				),
				mappedDir: {
					"app.js.map": await readfile(
						r.workspacePath("web/pathMapped/app.js.map")
					),
				},
			});
	...
	describe("launched", () => {
      ...
      `,
		...
		itIntegrates(
			"sets breakpoints in sourcemapped node_modules",
			async ({ r }) => {
				await r.initialize;
				...
				breakpoints: [
					{
						line: 32,
						column: 100,
						logMessage: "doubled: {$returnValue * 2}",
					},
				],
			});
			p.load();
				...
				breakpoints: [
					{
						...
						column: 0,
						logMessage:
							"{'LOG' + (window.logValue = (window.logValue || 0) + 1)}",
					},
				],
			});
			p.cdp.Runtime.evaluate({
				expression: "g(); console.log('DONE' + window.logValue)",
			});
			const outputs: Dap.OutputEventParams[] = [];
			for (let i = 0; i < 2; i++) {
				outputs.push(await p.dap.once("output"));
	...
	describe("custom", () => {
		itIntegrates("inner html", async ({ r }) => {
			...
			await p.evaluate(
				`document.querySelector('div').innerHTML = 'foo';`
			);

			p.log("Pausing on innerHTML");
			await p.dap.enableCustomBreakpoints({
	...
	describe("hot-transpiled", () => {
		...
		itIntegrates(
			"adjusts breakpoints after already running (#524)",
			async ({ r }) => {
				await r.initialize;
					...
					"//# sourceMappingURL=data:application/json;charset=utf-8;base64," +
						Buffer.from(
							JSON.stringify({
								...
								mappings:
									"AAAAA,OAAO,CAACC,GAAG,CAAC,aAAa,CAAC",
							})
						).toString("base64"),
				],
			});
	...
	itIntegrates(
		"gets correct line number with babel code (#407)",
		async ({ r }) => {
			await r.initialize;
	...
	describe("hit count", () => {
		const doTest = async (
			r: TestRoot,
			run: (p: TestP, source: Dap.Source) => Promise<void>
		) => {
			const p = await r.launchUrlAndLoad("index.html");
			p.cdp.Runtime.evaluate({
				expression: `
      ...
      `,
		...
		itIntegrates(
			"does not validate or hit invalid breakpoint",
			async ({ r }) => {
				await doTest(r, async (p, source) => {
					const output = p.dap.once("output");
					r.log(
	...
	itIntegrates(
		"stop on entry on .ts file reports as entry",
		async ({ r }) => {
			await r.initialize;
	...
	itIntegrates(
		"reevaluates breakpoints when new sources come in (#600)",
		async ({ r }) => {
	...
	itIntegrates(
		"can step in when first line of code is function",
		async ({ r }) => {
			createFileTree(testFixturesDir, {
				"test.js": [
					"function double(x) {",
	...
	itIntegrates("normalizes webpack nul byte (#1080)", async ({ r }) => {
		...
		const handle = await r.runScript(join(cwd, "build/greeter.js"), {
			cwd,
		});
		await handle.dap.setBreakpoints({
	...
	itIntegrates("stepInTargets", async ({ r }) => {
		...
		// line 3
		{
			...
			const { stackFrames } = await p.dap.stackTrace({
				threadId: threadId1,
			});
	...
	itIntegrates(
		"deals with removed execution contexts (#1582)",
		async ({ r }) => {
	...
	itIntegrates(
		"sets file uri breakpoints predictably (#1748)",
		async ({ r }) => {
			createFileTree(testFixturesDir, {
				"pages/main.html":
			...
			const p = await r.launchUrl(
				absolutePathToFileUrlWithDetection(mainFile),
				{
					url: undefined,
					file: mainFile,
				}
			...
			const source: Dap.Source = {
				path: join(testFixturesDir, "scripts/hello.js"),
			};
