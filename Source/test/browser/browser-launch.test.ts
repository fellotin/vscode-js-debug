...
describe("constructInspectorWSUri", () => {
	...
	it("interpolates arguments to construct inspectUri", () => {
		expect(
			constructInspectorWSUri(inspectUri, appHttpUrl, browserWsInspectUri)
		).to.be.eq(
	...
	it("fails with an useful error for invalid urls", () => {
		...
		expect(() =>
			constructInspectorWSUri(inspectUri, null, browserWsInspectUri)
		).to.throw(`A valid url wasn't supplied: <null>`);
		expect(() =>
			constructInspectorWSUri(inspectUri, undefined, browserWsInspectUri)
		).to.throw(`A valid url wasn't supplied: <undefined>`);
		expect(() =>
			constructInspectorWSUri(inspectUri, "", browserWsInspectUri)
		).to.throw(`A valid url wasn't supplied: <>`);
	});
