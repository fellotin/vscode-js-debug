...
describe("stacks", () => {
	...
	describe("smartStep", () => {
		...
		itIntegrates("remembers step direction out", async ({ r }) => {
			...
			await p.dap.setBreakpoints({
				source: {
					path: p.workspacePath("web/smartStep/directional.ts"),
				},
		...
		itIntegrates(
			"does not smart step on exception breakpoints",
			async ({ r }) => {
				const p = await r.launchUrlAndLoad("index.html");
		...
		itIntegrates(
			"does not smart step manual breakpoints",
			async ({ r }) => {
				const p = await r.launchUrlAndLoad("index.html");
				await p.dap.setBreakpoints({
					source: {
						path: p.workspacePath("web/smartStep/exceptionBp.js"),
					},
	...
	describe("skipFiles", () => {
		async function waitForPausedThenDelayStackTrace(
			p: TestP,
			scopes: boolean
		) {
			const event = await p.dap.once("stopped");
			await delay(200); // need to pause test to let debouncer update scripts
			await p.logger.logStackTrace(
				event.threadId!,
				scopes ? Infinity : 0
			);
			return event;
