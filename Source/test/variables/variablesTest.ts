...
describe("variables", () => {
	...
	describe("object", () => {
		itIntegrates("simple array", async ({ r }) => {
			const p = await r.launchAndLoad("blank");
			await p.logger.evaluateAndLog("var a = [1, 2, 3]; a.foo = 1; a", {
		...
		itIntegrates.skip("large array", async ({ r }) => {
			...
			await p.logger.evaluateAndLog(
				"var a = new Array(110); a.fill(1); a",
				{
					logInternalInfo: true,
				}
			);
		...
		describe("customDescriptionGenerator", () => {
          ...
          new Bar();`);
			...
			itIntegrates(
				"using statement with return syntax",
				async ({ r }) => {
					const p = await r.launchAndLoad("blank", {
						customDescriptionGenerator:
		...
		describe("customPropertiesGenerator", () => {
			itIntegrates(
				"works with customPropertiesGenerator method ",
				async ({ r }) => {
					const p = await r.launchAndLoad("blank", {
		...
		itIntegrates(
			"shows errors while generating properties",
			async ({ r }) => {
				const p = await r.launchAndLoad("blank", {
					customPropertiesGenerator:
	...
	describe("multiple threads", () => {
		itIntegrates("worker", async ({ r }) => {
			...
			await Promise.all([
				(async () =>
					outputs.push({
						output: await p.dap.once("output"),
						logger: p.logger,
					}))(),
				(async () => {
					const worker = await r.worker();
					outputs.push({
						output: await worker.dap.once("output"),
	...
	describe("setVariable", () => {
		itIntegrates("basic", async ({ r }) => {
			const p = await r.launchAndLoad("blank");
			const v = await p.logger.evaluateAndLog(
				`window.x = ({foo: 42}); x`
			);
      ...
      `,
			...
			const stack = await p.dap.stackTrace({
      ...
      `,
			...
			const scopes = await p.dap.scopes({
				frameId: stack.stackFrames[0].id,
			});
		...
		itIntegrates("name mapping", async ({ r }) => {
			...
			const stacks = await p.logger.logStackTrace(
				event.threadId!,
				Infinity
			);
