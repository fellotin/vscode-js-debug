...
describe("resourceProvider", () => {
	async function waitForPause(
		p: ITestHandle,
		cb?: (threadId: string) => Promise<void>
	) {
	...
	it("decodes base64 data uris", async () => {
		...
		expect(
			await rp.fetch("data:text/plain;base64,SGVsbG8gd29ybGQh")
		).to.deep.equal({
			ok: true,
			statusCode: 200,
			body: "Hello world!",
	...
	it("decodes utf8 data uris (#662)", async () => {
		const rp = new BasicResourceProvider(fs);
		expect(
			await rp.fetch("data:text/plain;utf-8,Hello%20world!")
		).to.deep.equal({
			ok: true,
			statusCode: 200,
			body: "Hello world!",
	...
	it("follows redirects (unit)", async () => {
		const rp = new BasicResourceProvider(fs);
		expect(
			await rp.fetch("http://localhost:8001/redirect-to-greet")
		).to.deep.equal({
			ok: true,
			statusCode: 200,
			body: "Hello world!",
	...
	it("applies request options", async () => {
		...
		expect(JSON.parse(res.body || "no content")).to.containSubset({
			cool: "true",
		});
	});
});
