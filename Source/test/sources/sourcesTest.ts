...
describe("sources", () => {
	async function dumpSource(
		p: ITestHandle,
		event: Dap.LoadedSourceEventParams,
		name: string
	) {
	...
	itIntegrates("basic sources", async ({ r }) => {
		...
		await dumpSource(
			p,
			await p.waitForSource("doesnotexist"),
			"does not exist"
		);
		p.addScriptTag("dir/helloworld.js");
		await dumpSource(
			p,
			await p.waitForSource("helloworld"),
			"dir/helloworld"
		);
		p.evaluate("42", "");
	...
	itIntegrates("supports nested sourcemaps (#1390)", async ({ r }) => {
		...
		const handle = await r.runScript(join(cwd, "a/main.bundle.js"), {
			cwd,
		});
		await handle.dap.setBreakpoints({
			source: {
				path: join(testWorkspace, "nestedSourceMaps", "b", "lib.js"),
			},
	...
	itIntegrates(
		"works with relative webpack sourcemaps (#479)",
		async ({ r }) => {
	...
	itIntegrates(
		"allows overrides for relative webpack paths (#479)",
		async ({ r }) => {
			const p = await r.launchUrl("webpack/relative-paths.html", {
	...
	itIntegrates("allows module wrapper in node code", async ({ r }) => {
		const handle = await r.runScript(
			join(testWorkspace, "moduleWrapper", "index.js")
		);
		handle.load();
	...
	itIntegrates(
		"verifies content when enableContentValidation=true",
		async ({ r }) => {
			const handle = await r.runScript(
				join(testWorkspace, "moduleWrapper", "customWrapper.js")
			);
			handle.load();
			const src = await handle.waitForSource("moduleWrapper/test.js");
	...
	itIntegrates(
		"does not verify content when enableContentValidation=false",
		async ({ r }) => {
			const handle = await r.runScript(
				join(testWorkspace, "moduleWrapper", "customWrapper.js"),
				{
	...
	itIntegrates("removes any query from node paths (#529)", async ({ r }) => {
		...
		handle.load();
		handle.log(
			await handle.waitForSource("simpleWebpackWithQuery.ts"),
			undefined,
			[]
		);
		handle.assertLog();
	...
	describe("sourcemap error handling", () => {
		...
		itIntegrates("logs lazy parse errors", async ({ r }) => {
			...
			const output = r
				.rootDap()
				.once("output", (o) => o.category === "stderr");
