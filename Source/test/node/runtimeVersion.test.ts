...
describe("runtimeVersion", () => {
	...
	it("fails if no nvm/s present", async () => {
		...
		await expect(
			resolver.resolveNvmVersionPath("13")
	...
	describe("fall throughs", () => {
		...
		it("attempts multiple lookup to get the right version", async () => {
			const { directory: a } =
				await resolver.resolveNvmVersionPath("13.12");
			expect(a).to.equal(
				path.join(testFixturesDir, "nvs/node/13.12.0/x64/bin")
			);

			const { directory: b } =
				await resolver.resolveNvmVersionPath("13.11");
			expect(b).to.equal(
				path.join(testFixturesDir, "nvm/versions/node/v13.11.0/bin")
			);

			const { directory: c } =
				await resolver.resolveNvmVersionPath("13.10");
			expect(c).to.equal(
				path.join(
					testFixturesDir,
					"fnm/node-versions/v13.10.0/installation/bin"
				)
			);

			await expect(
				resolver.resolveNvmVersionPath("14")
			).to.eventually.be.rejectedWith(
				ProtocolError,
		...
		it("requires nvs for a specific architecture", async () => {
			...
			await expect(
				resolver.resolveNvmVersionPath("13.11/x64")
			).to.eventually.be.rejectedWith(
				ProtocolError,
		...
		it("does not fall through if requesting a specific nvs architecture", async () => {
			await expect(
				resolver.resolveNvmVersionPath("13.11/x64")
			).to.eventually.be.rejectedWith(ProtocolError, /not installed/);
		});
	});
	...
	describe("nvs support", () => {
		...
		it("gets an exact match", async () => {
			const { directory, binary } =
				await resolver.resolveNvmVersionPath("13.3.0");
			expect(directory).to.equal(
				path.join(testFixturesDir, "node/13.3.0/x64/bin")
			);
		...
		it("resolves node64", async () => {
			const { directory, binary } =
				await resolver.resolveNvmVersionPath("13.3.1");
			expect(directory).to.equal(
				path.join(testFixturesDir, "node/13.3.1/x64/bin")
			);
		...
		it("gets the best matching version", async () => {
			...
			expect(directory).to.equal(
				path.join(testFixturesDir, "node/13.12.0/x64/bin")
			);
		});
		...
		it("throws on none for specific architecture", async () => {
			await expect(
				resolver.resolveNvmVersionPath("13.4.0")
			).to.eventually.be.rejectedWith(ProtocolError, /not installed/);
		});

		it("gets a specific architecture", async () => {
			const { directory } =
				await resolver.resolveNvmVersionPath("13/x86");
			expect(directory).to.equal(
				path.join(testFixturesDir, "node/13.4.0/x86/bin")
			);
		});
		...
		it("omits the bin directory on windows", async () => {
			...
			const { directory } =
				await resolver.resolveNvmVersionPath("13.3.0");
			expect(directory).to.equal(
				path.join(testFixturesDir, "node/13.3.0/x64")
			);
		});
	});
	...
	describe("nvm osx", () => {
		...
		it("gets an exact match", async () => {
			const { directory, binary } =
				await resolver.resolveNvmVersionPath("13.3.0");
			expect(directory).to.equal(
				path.join(testFixturesDir, "versions/node/v13.3.0/bin")
			);
			expect(binary).to.equal("node");
		});
		...
		it("resolves node64", async () => {
			const { directory, binary } =
				await resolver.resolveNvmVersionPath("13.3.1");
			expect(directory).to.equal(
				path.join(testFixturesDir, "versions/node/v13.3.1/bin")
			);
			expect(binary).to.equal("node64");
		});
		...
		it("gets the best matching version", async () => {
			const { directory } = await resolver.resolveNvmVersionPath("13");
			expect(directory).to.equal(
				path.join(testFixturesDir, "versions/node/v13.12.0/bin")
			);
		});
	...
	describe("fnm", () => {
		...
		it("gets an exact match", async () => {
			...
			expect(directory).to.equal(
				path.join(
					testFixturesDir,
					"node-versions/v13.3.0/installation/bin"
				)
			);
			expect(binary).to.equal("node");
		});
		...
		it("gets the best matching version", async () => {
			...
			expect(directory).to.equal(
				path.join(
					testFixturesDir,
					"node-versions/v13.12.0/installation/bin"
				)
			);
		});
