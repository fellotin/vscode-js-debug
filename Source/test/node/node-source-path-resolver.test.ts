...
describe("node source path resolver", () => {
	describe("url to path", () => {
		...
		it("resolves absolute", async () => {
			...
			expect(
				await r.urlToAbsolutePath({ url: "file:///src/index.js" })
			).to.equal(resolve("/src/index.js"));
		});
		...
		it("escapes regex parts segments", async () => {
			if (process.platform === "win32") {
				...
				expect(
					(r as unknown as Record<string, string[]>).resolvePatterns
				).to.deep.equal([
					"C:/some/workspa\\*ce/folder/**",
					"C:/some/workspa\\*ce/**",
			...
			if (process.platform === "win32") {
				...
				expect(
					r.shouldResolveSourceMap({
						compiledPath:
							"c:\\Users\\Segev\\prj\\swimm\\ide\\server\\dist\\app.js",
						sourceMapUrl:
							"file:///c:/Users/Segev/prj/swimm/ide/server/dist/app.js.map",
					})
				).to.be.true;
			}
		});
		...
		it("resolves unc paths", async () => {
			...
			expect(
				...
				resolve("\\\\mac\\Home\\Github\\js-debug-demos\\node\\main.js")
			);
		});
		...
		it("normalizes roots (win -> posix) ", async () => {
			...
			expect(
				await r.urlToAbsolutePath({
					url: "file:///c:/source/foo/bar.js",
				})
			).to.equal("/dev/src/foo/bar.js");
		});
		...
		it("normalizes roots (posix -> win) ", async () => {
			...
			expect(
				await r.urlToAbsolutePath({ url: "file:///dev/src/foo/bar.js" })
			).to.equal("c:\\Source\\foo\\bar.js");
		});
		...
		it("applies source map overrides", async () => {
			...
			expect(
				await r.urlToAbsolutePath({
					...
					map: {
						sourceRoot: "",
						metadata: { compiledPath: "hello.js" },
					} as any,
				})
		...
		it("loads local node internals (#823)", async () => {
			...
			expect(
				await r.urlToAbsolutePath({ url: "node:internal/url.js" })
			).to.equal(join(__dirname, "lib/internal/url.js"));
		});
		...
		describe("source map filtering", () => {
			...
			for (const key of Object.keys(testTable)) {
				...
				const caseSensitive =
					"caseSensitive" in tcase && tcase.caseSensitive;
				...
				it(key, async () => {
					...
					const result = await resolver.urlToAbsolutePath({
						url: "webpack:///hello.js",
						map: {
							metadata: { sourceMapUrl: map, compiledPath: map },
							sourceRoot: "",
						} as any,
					});
