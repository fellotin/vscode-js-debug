...
const assertParses = async (
	...
	const stubbed = stub(tree, "createProcess" as any).returns(
		fakeChildProcess(input)
	);
	try {
		const result = await tree.lookup<IProcess[]>(
			(entry, acc) => [...acc, entry],
			[]
		);
...
describe("process tree", () => {
	it("gives some output for the current platform", async () => {
		// sanity check
		const data = await processTree.lookup<IProcess[]>(
			(entry, acc) => [...acc, entry],
			[]
		);
	...
	if (process.platform !== "win32") {
		it("gets the working directory", async () => {
			const currentWd = await processTree.getWorkingDirectory(
				process.pid
			);
		...
		await assertParses(
			...
			[
				...
				},
				{
					pid: 383,
					ppid: 1,
					command: "usr/sbin/distno",
					args: "/usr/sbin/distnoted agent",
				},
				{
					pid: 384,
					ppid: 1,
					command: "usr/libexec/USB",
					args: "/usr/libexec/USBAgent",
				},
				{
					pid: 387,
					ppid: 1,
				...
				},
				{
					...
					args: "/usr/libexec/lsd",
				},
				{
					pid: 390,
					ppid: 1,
					command: "usr/libexec/tru",
					args: "/usr/libexec/trustd --agent",
				},
				{
					pid: 391,
					ppid: 1,
					command: "usr/libexec/sec",
					args: "/usr/libexec/secd",
				},
				{
					pid: 392,
					ppid: 2,
