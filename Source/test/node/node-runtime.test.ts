...
describe("node runtime", () => {
	...
	itIntegrates("chakracore string value", async ({ r }) => {
		...
		const chakracore = join(
			testWorkspace,
			"chakracore",
			"ChakraCore.Debugger.Sample.exe"
		);
		...
		itIntegrates("debugs worker threads", async ({ r }) => {
			...
			createFileTree(testFixturesDir, {
				"test.js": [
					...
					"if (isMainThread) {",
					"  new Worker(__f" +
						'ilename, { workerData: { greet: "world" } });',
					"} else {",
					"  setInterval(() => {",
	...
	itIntegrates("exits with integrated terminal launcher", async ({ r }) => {
		...
		const launch = stub(
			TerminalProgramLauncher.prototype,
			"sendLaunchRequest"
		);
	...
	describe("stopOnEntry", () => {
		...
		itIntegrates(
			"stops with a breakpoint elsewhere (#515)",
			async ({ r }) => {
				const handle = await r.runScript("test.js", {
					cwd: testFixturesDir,
					stopOnEntry: true,
	...
	describe("attaching", () => {
		...
		itIntegrates("does not restart if killed", async ({ r }) => {
			...
			child = spawn(
				"node",
				[`--inspect=${port}`, join(testFixturesDir, "test")],
				{
					stdio: "pipe",
				}
			);
			...
			await delay(1000);
			expect(
				lines.filter((l) => l.includes("Debugger attached"))
			).to.have.lengthOf(1);
		});
	});
	...
	itIntegrates("sets sourceMapOverrides from the cwd", async ({ r }) => {
		const handle = await r.runScript(
			join(testWorkspace, "simpleNode", "simpleWebpack.js"),
			{
	...
	itIntegrates(
		"writes errors if runtime executable not found",
		async ({ r }) => {
			await r.initialize;
			...
			expect(result).to.include(
				'Can\'t find Node.js binary "does-not-exist"'
			);
		}
	);
	...
	itIntegrates("scripts with http urls", async ({ r }) => {
		...
		const handle = await r.runScript(join(cwd, "index.js"), {
			...
			sourceMapPathOverrides: {
				"http://localhost:8001/*": `${testWorkspace}/web/*`,
			},
		});
		handle.load();
		await waitForPause(handle);
	...
	describe("simplePortAttach", () => {
		...
		itIntegrates("allows inspect-brk in npm scripts", async ({ r }) => {
			...
			const optionsOut = handle.dap.once("output", (o) =>
				o.output.includes("NODE_OPTIONS")
			);
			handle.load();
			const { threadId } = handle.log(await handle.dap.once("stopped"));
			handle.dap.continue({ threadId });
		...
		itIntegrates(
			"uses bootloader for normal npm scripts",
			async ({ r }) => {
				await r.initialize;
				const cwd = join(testWorkspace, "simpleNode");
				...
				const worker = await r.worker();
				const optionsOut = worker.dap.once("output", (o) =>
					o.output.includes("NODE_OPTIONS")
				);
				handle.logger.logOutput(await optionsOut);
				handle.assertLog({
					customAssert: (l) =>
						expect(l).to.contain("NODE_OPTIONS=  --require"),
				});
			}
		);
