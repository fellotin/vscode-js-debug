...
describe("profiling", () => {
	...
	describe("console", () => {
		itIntegrates("works", async ({ r }) => {
			createFileTree(testFixturesDir, {
				"test.js": [
					'console.profile("some-profile");',
					'console.profileEnd("some-profile");',
				],
			});
			...
			await eventuallyOk(() =>
				assertValidOutputFile(
					join(testFixturesDir, "some-profile.cpuprofile")
				)
			);
		});
	});
	...
	describe("cpu-profiling", () => {
		...
		describe("breakpoints", () => {
			...
			itIntegrates(
				"continues if was paused on start with debugger domain",
				async ({ r }) => {
					await r.initialize;
			...
			itIntegrates("unverifies and reverifies", async ({ r }) => {
				...
				const logfn = stub().callsFake((data) =>
					handle.log(data, undefined, [])
				);
				handle.dap.on("breakpoint", logfn);

				await handle.dap.startProfile({ type: "cpu" });
				await eventuallyOk(
					() => expect(logfn.callCount).to.gte(1),
					2000
				);
				await handle.dap.stopProfile({});
				await eventuallyOk(
					() => expect(logfn.callCount).to.gte(2),
					2000
				);
				handle.assertLog();
			});
			...
			itIntegrates(
				"does not unverify target breakpoint",
				async ({ r }) => {
					await r.initialize;
		...
		describe("ui", () => {
			...
			const pickTermination = async (
				...
				typePicker.selectedItems = typePicker.items.filter((i) =>
					/CPU/i.test(i.label)
				);
				...
				terminationPicker.selectedItems =
					terminationPicker.items.filter((i) =>
			...
			it.skip("allows picking breakpoints", async () => {
				...
				const session = await new Promise<vscode.DebugSession>(
					(resolve) =>
						vscode.debug.onDidStartDebugSession((s) =>
							"__pendingTargetId" in s.configuration
								? resolve(s)
								: undefined
						)
				);
				...
				const breakpointPicker = await eventuallyOk(() => {
					...
					expect(picker.items.length).to.be.greaterThan(
						0,
						"expected to have picker items"
					);
					return picker;
				}, 5000);
			...
			it("sets substate correctly", async () => {
				...
				await eventuallyOk(
					() => expect(session.name).to.contain("Profiling"),
					2000
				);
				await runCommand(
					vscode.commands,
					Commands.StopProfile,
					session.id
				);
				await eventuallyOk(
					() => expect(session.name).to.not.contain("Profiling"),
					2000
				);
				disposable.dispose();
			});
	...
	describe("heap-profiling", () => {
		itIntegrates("heap sanity test", async ({ r }) => {
			await r.initialize;
		...
		describe("ui", () => {
			...
			const pickTermination = async (
				...
				// we skip this step while "heap" is the only profile:
				const typePicker = await eventuallyOk(() => {
					expect(createQuickPick.callCount).to.equal(1);
					const picker: vscode.QuickPick<vscode.QuickPickItem> =
						createQuickPick.getCall(0).returnValue;
			...
			it("works with pure command API", async () => {
				...
				expect(() => JSON.parse(args.contents)).to.not.throw;
				expect(args.basename).to.match(/\.heapprofile$/);
				disposable.dispose();
			});
