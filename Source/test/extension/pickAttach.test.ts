...
describe("pick and attach", () => {
	...
	if (process.platform !== "win32") {
		...
		it("limits inference to workspace root", async () => {
			...
			const getWorkspaceFolder = sandbox.stub(
				vscode.workspace,
				"getWorkspaceFolder"
			);
			...
			await resolveProcessId(new LocalFsUtils(fsPromises), config, true);
			expect(removePrivatePrefix(config.cwd!)).to.equal(
	...
	describe("", () => {
		beforeEach(async () => {
			...
			child = spawn("node", ["--inspect-brk", `--inspect-port=${port}`], {
				stdio: "pipe",
			});
			...
			child
				...
				.on(
					...
					(line: string) =>
						(attached =
							attached || line.includes("Debugger attached"))
				);
		});
		...
		it("end to end", async function () {
			...
			const createQuickPick = sandbox.spy(
				vscode.window,
				"createQuickPick"
			);
			...
			const item = await eventuallyOk(() => {
				const i = picker.items.find(
					(item) =>
						(item as any).pidAndPort === `${child.pid}:${port}`
				);
				if (!i) {
			...
			await eventuallyOk(
				() =>
					expect(attached).to.equal(
						true,
						"expected to have attached"
					),
				10 * 1000
			);
		});
