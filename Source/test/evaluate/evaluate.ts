...
describe("evaluate", () => {
	itIntegrates("default", async ({ r }) => {
		...
		await p.logger.evaluateAndLog(`Object.create({ set foo(x) {} })`, {
			depth: 2,
		});
		p.log("");

		await p.logger.evaluateAndLog(
			`Object.create({ get foo() { return 42 } })`,
			{ depth: 2 }
		);
		p.log("");

		await p.logger.evaluateAndLog(
			`Object.create({ get foo() { throw 'wat'; } })`,
			{ depth: 2 }
		);
		p.log("");

		await p.logger.evaluateAndLog(
			`Object.create({ get [Symbol.toStringTag]() { return 42 } })`,
			{
				depth: 2,
			}
		);
		...
		p.dap.evaluate({
			expression: `setTimeout(() => { throw new Error('baz')}, 0)`,
		});
		await p.logger.logOutput(await p.dap.once("output"));
		p.log("");
		...
		await p.logger.evaluateAndLog(
			`window.throwValue({foo: 3, bar: 'baz'})`
		);
		p.log("");

		p.dap.evaluate({
			expression: `setTimeout(() => { window.throwError('error2')}, 0)`,
		});
		await p.logger.logOutput(await p.dap.once("output"));
		p.log("");
	...
	itIntegrates("valueFormatter", async ({ r }) => {
		...
		await p.logger.evaluateAndLog(
			`123567891235678912356789123567891235678912356789n`,
			{ format }
		);
		p.log("");
		...
		await p.logger.evaluateAndLog(`({ a: 'hello', b: 42, c: true })`, {
			format,
		});
		p.log("");
  ...
  recurse: [Circular],
	...
	itIntegrates("copy via evaluate context", async ({ r }) => {
		const p = await r.launchAndLoad("blank");
		for (const [expression, expected] of Object.entries(copyExpressions)) {
			const actual = await p.dap.evaluate({
				expression,
				context: "clipboard",
			});
    ...
    `);
		...
		for (const expression of exprs) {
			...
			p.logger.logEvaluateResult(
				await p.dap.evaluate({ expression, context: "repl" }),
				{
					depth: 0,
				}
			);
	...
	itIntegrates("selected context", async ({ r }) => {
		...
		p.log("Pausing...");
		p.dap.evaluate({
			expression: `window.w.postMessage('pause');`,
			context: "repl",
		});
		...
		p.dap.evaluate({
			expression: `window.w.postMessage('pauseWorker');`,
			context: "repl",
		});
