 ...
 */
export class Session<TSessionImpl extends IDebugSessionLike>
...
export class RootSession<
	TSessionImpl extends IDebugSessionLike,
> extends Session<TSessionImpl> {
	...
	public getOrLaunchSession(target: ITarget): Promise<Session<TSessionImpl>> {
		...
		const newSession = new Promise<Session<TSessionImpl>>(
			async (fulfill, reject) => {
				...
				this._pendingTarget.set(target.id(), {
					target,
				...
				const config: IPseudoAttachConfiguration = {
					...
					preRestartTask:
						parentConfig.preRestartTask ??
						parentConfig.postDebugTask,
					postRestartTask:
						parentConfig.postRestartTask ??
						parentConfig.preLaunchTask,
				};
