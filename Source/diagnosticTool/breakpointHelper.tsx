...
const buildTracing = (bp: IDiagnosticBreakpoint, dump: IDiagnosticDump) => {
	...
	const steps = [
		<li key={key++}>
			...
			<p>
				<code>{bp.source.path}</code> line {bp.params.line} column{" "}
				{bp.params.column || 1}
			</p>
		</li>,
	];
	...
	steps.push(
		<li key={key++}>
			<p>
				✅ The runtime acknowledged and adjusted the breakpoint, and it
				mapped back to the following locations:
			</p>
			<ul>
				{uiLocations.map((l, i) => (
		...
		<li key={key++}>
			<p>
				If this is not right, your compiled code might be out of date
				with your sources. If you don't think this is the case and
				something else is wrong, please{" "}
...
const NoUiLocation: FunctionComponent = () => {
	...
	return (
		<p>
			❓ We sent the breakpoint, but it didn't bind to any locations. If
			this is unexpected:
			<ul>
				<li>
					Make sure that your program is loading or running this
					script. You can add a <code>debugger;</code> statement to
					check this: your program will pause when it hits it.
				</li>
				<li>
					If your breakpoint is set in certain places, such as on the
					last empty line of a file, the runtime might not be able to
					find anywhere to place it.
				</li>
				{isNodeType(dump) && (
					<li>
						Unless you{" "}
						...
						, Node.js might not resolve breakpoints for code it
						hasn't parsed yet.
					</li>
				)}
				<li>
					If necessary, make sure your compiled files are up-to-date
...
const Breakpoint: FunctionComponent<{ bp: IDiagnosticBreakpoint }> = ({
	bp,
}) => {
	if (!bp.source.path) {
		return null;
...
const FailedToSetLocation: FunctionComponent<{ bp: IDiagnosticBreakpoint }> = ({
	bp,
}) => {
	const dump = useDump();
	...
	return (
		<li>
			<p>
				❓ We couldn't find a corresponding source location, but found
				some other files with the same name:
			</p>
			<ul>
				{matchingSources.map((s) => (
					<li key={s}>
						<TextDiff
							original={bp.source.path as string}
							updated={s.absolutePath || s.url}
						/>
					</li>
				))}
			</ul>
			{isBrowserType(dump) ? (
				<p>
					You may need to adjust the <code>webRoot</code> in your{" "}
					<code>launch.json</code> if you're building from a
					subfolder, or tweak your <code>sourceMapPathOverrides</code>
					.
				</p>
			) : (
				<p>
					If this is the same file, you may need to adjust your build
					tool{" "}
					{isBrowserType(dump) && (
						<Fragment>
...
const TextDiff: FunctionComponent<{ original: string; updated: string }> = ({
	...
	<span className="text-diff">
		{diffArrays(original.split(/[/\\]/g), updated.split(/[/\\]/g), {
			ignoreCase: true,
		}).map((diff, i) => (
			<span
...
const UiLocation: FunctionComponent<{ loc: IDiagnosticUiLocation }> = ({
	loc,
}) => {
	const dump = useDump();
	...
	return (
		<Fragment>
			<code>{source?.absolutePath ?? source?.url ?? "unknown"}</code> line{" "}
			{loc.lineNumber} column {loc.columnNumber}
		</Fragment>
	);
};

const CdpBreakpoint: FunctionComponent<{
	...
	return (
		<li>
			<p>
				...
				{regex && (
					<a onClick={() => setShowRegex(!showRegex)}>
						via this regex
					</a>
				)}
			</p>
...
const NoMatchingSourceHelper: FunctionComponent<{ basename: string }> = ({
	basename,
}) => {
	const dump = useDump();
	...
	return (
		<Fragment>
			<p>
				❓ We couldn't find a corresponding source location, and didn't
				find any source with the name <code>{basename}</code>.
			</p>
			<p>
				How did you expect this file to be loaded? (If you have a
				compilation step, you should pick 'sourcemap')
				<NoMatchingDecisionButtons onChange={setHint} value={hint} />
				{hint === NoMatchingSourceHint.Direct &&
					(isBrowserType(dump) ? (
						<p>
							It looks like your webpage didn't load this script;
							breakpoints won't be bound until the file they're
							set in is loaded. Make sure your script is imported
							from the right location using a{" "}
							<code>{"<script>"}</code> tag.
						</p>
					) : (
						<p>
							It looks like your program didn't load this script;
							breakpoints won't be bound until the file they're
							set in is loaded. Make sure your script is imported
							with a <code>require()</code> or <code>import</code>{" "}
							statement, such as{" "}
					...
					<p>
						...
						<ul>
							{/\.tsx?$/.test(basename) ? (
								<li>
									Make sure you have{" "}
									<code>"sourceMap": true</code> in your
									tsconfig to generate sourcemaps.
								</li>
							) : (
								<li>
									Make sure your build tool is set up to
									create sourcemaps.
								</li>
							)}
							{!dump.config.outFiles.includes(
								...
								<li>
									...
									<code>outFiles</code> in your launch.json.
									Try removing this: it now defaults to the
									whole workspace, and overspecifying it can
									unnecessarily narrow places where we'll
									resolve sourcemaps.
								</li>
							)}
						</ul>
