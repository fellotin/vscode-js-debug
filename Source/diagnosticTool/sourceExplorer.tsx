		...
		() =>
			dump.sources
				.map(
					(source) =>
						[
							[source.url, source.absolutePath, source.prettyName]
								.join(" ")
								.toLowerCase(),
							source,
	...
	return (
		<Fragment>
			...
			{results.map((result) => (
				<Source
					source={result}
					allSources={uniqueIdMap}
					key={result.sourceReference}
				/>
			))}
		</Fragment>
	);
...
const SourceData: FunctionComponent<{
	...
	<dl className="source-data-grid">
		...
		<dd>
			{source.sourceMap ? (
				<ul>
					{Object.entries(source.sourceMap.sources).map(
						([url, ref]) => (
							<li key={url}>
								<ReferencedSource
									url={url}
...
const ReferencedSource: FunctionComponent<{
	...
	return (
		<Fragment>
			{url} &rarr;{" "}
			<a onClick={onClick}>
				{src ? `${basename(src)} (#${sourceRef})` : "unknown"}
			</a>
		</Fragment>
	);
};
