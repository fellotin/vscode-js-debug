...
} from "./adapter/exceptionPauseService";
import {
	IPerformanceProvider,
	PerformanceProviderFactory,
} from "./adapter/performance";
import {
	IPortLeaseTracker,
	PortLeaseTracker,
...
import {
	IProfileController,
	ProfileController,
} from "./adapter/profileController";
...
import {
	...
	IDefaultBrowserProvider,
} from "./common/defaultBrowserProvider";
...
import {
	MutableLaunchConfig,
	createMutableLaunchConfig,
} from "./common/mutableLaunchConfig";
import {
	IRenameProvider,
	RenameProvider,
} from "./common/sourceMaps/renameProvider";
import {
...
import {
	...
	PackageJsonProvider,
} from "./targets/node/packageJsonProvider";
...
import {
	IShutdownParticipants,
	ShutdownParticipants,
} from "./ui/shutdownParticipants";
import {
	registerTopLevelSessionComponents,
...
export const createTargetContainer = (
	...
	if (target.sourcePathResolver) {
		container
			...
			.toConstantValue(target.sourcePathResolver);
	}
...
export const createTopLevelSessionContainer = (parent: Container) => {
	...
	const browserFinderFactory =
		(ctor: BrowserFinderCtor) => (ctx: interfaces.Context) =>
			new ctor(
