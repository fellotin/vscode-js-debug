...
export class DapTelemetryReporter implements ITelemetryReporter {
	 ...
	 * @inheritdoc
	 */
	public report<K extends keyof LogFunctions>(
		key: K,
		...args: Parameters<LogFunctions[K]>
	) {
	...
	public flush() {
		...
		const pending = Object.entries(this.batchFlushTimeout) as [
			Batchable,
			NodeJS.Timeout,
		][];
 ...
 */
export function extractErrorDetails(e: Error): {
