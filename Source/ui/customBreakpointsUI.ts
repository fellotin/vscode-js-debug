...
class BreakpointsDataProvider
	implements vscode.TreeDataProvider<Breakpoint | Category>
{
	...
	/** @inheritdoc */
	getChildren(
		item?: Breakpoint | Category
	): vscode.ProviderResult<(Breakpoint | Category)[]> {
		if (!item) {
			return [...this.categories.values()];
		}
		...
		for (const [breakpoint, enabled] of breakpoints) {
			...
			if (breakpoint instanceof Category) {
				...
				if (!enabled && breakpoint.parent.checked) {
					...
					enabled &&
					breakpoint.parent.children.every((c) => c.checked)
				) {
					breakpoint.parent.checkboxState = state;
				}
			}
...
export function registerCustomBreakpointsUI(
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			() => {
				provider.setEnabled(
					provider.allBreakpoints.map((bp) => [bp, false])
				);
			}
		)
	);
