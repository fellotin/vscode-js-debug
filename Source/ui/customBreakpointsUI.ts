...
class BreakpointsDataProvider
	implements vscode.TreeDataProvider<Breakpoint | vscode.TreeItem>
{
	...
	constructor(debugSessionTracker: DebugSessionTracker) {
		this.breakpoints = [...customBreakpoints().values()].reduce(
			(acc, breakpoint) => {
				let list = acc.get(breakpoint.group);
				if (!list) {
					list = [];
					acc.set(breakpoint.group, list);
				}
				list.push(new Breakpoint(breakpoint, false));
				return acc;
			},
	...
	getChildren(
		...
		if (!item)
			return [...this.breakpoints].map(([key, breakpoint]) => {
				const retVal = new vscode.TreeItem(
					key,
					vscode.TreeItemCollapsibleState.Collapsed
				);
				retVal.checkboxState = breakpoint.every((p) => p.checkboxState)
					? 1
					: 0;
				return retVal;
			});
	...
	removeBreakpoints(breakpoints: Breakpoint[]) {
		...
		for (const session of this._debugSessionTracker.getConcreteSessions())
			session.customRequest("disableCustomBreakpoints", {
				ids: breakpoints.map((e) => e.id),
			});
...
export function registerCustomBreakpointsUI(
	...
	const view = vscode.window.createTreeView(
		CustomViews.EventListenerBreakpoints,
		{
	...
	view.onDidChangeCheckboxState((e) => {
		for (const item of e.items) {
			if (item[0] instanceof Breakpoint) {
				...
				const allBr = provider.breakpoints.get(
					item[0].label.toString()
				);
				if (!allBr) continue;
				if (item[1]) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			Commands.RemoveAllCustomBreakpoints,
			() => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			Commands.RemoveCustomBreakpoints,
			() => {
