 ...
 */
@injectable()
export abstract class ChromiumDebugConfigurationResolver<
		T extends AnyChromiumConfiguration,
	>
	...
	implements vscode.DebugConfigurationProvider
{
	...
	protected async resolveBrowserCommon(
		...
		if (config.server && "program" in config.server) {
			...
			config.server =
				await this.terminalProvider.resolveDebugConfiguration(folder, {
					...config.server,
