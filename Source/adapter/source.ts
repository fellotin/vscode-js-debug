...
export class Source {
	...
	public offsetScriptToSource<
		T extends { lineNumber: number; columnNumber: number },
	>(obj: T): T {
	 ...
	 */
	public async prettyPrint(): Promise<
		{ map: SourceMap; source: Source } | undefined
	> {
		if (!this._container) {
			return undefined;
		}
		...
		return {
			map,
			source: [...asCompiled.sourceMap.sourceByUrl.values()][0],
		};
	}
...
export namespace SourceLocationProvider {
	...
	export function waitForValueWithTimeout(
		...
		return Promise.race([
			waitForValue(p),
			delay(timeout) as Promise<undefined>,
		]);
	}
...
export interface ISourceWithMap<
	T extends SourceLocationProvider = SourceLocationProvider,
> extends Source {
	sourceMap: T;
}
/**
...
export class WasmSource
	extends Source
	implements ISourceWithMap<IWasmLocationProvider>
{
