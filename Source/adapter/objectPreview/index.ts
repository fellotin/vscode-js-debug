...
import { IPreviewContext, getContextForType } from "./contexts";
...
export function previewAsObject(
	...
	return (
		...
		(object.type === "object" &&
			!subtypesWithoutPreview.has(object.subtype))
	);
}
...
function formatFunctionDescription(
	...
	function nameAndArguments(contents: string): string {
		...
		if (
			...
			endOfArgumentsMatch &&
			endIndex > startOfArgumentsIndex
		) {
			const name =
...
export const messageFormatters: messageFormat.Formatters<ObjectPreview.AnyObject> =
	new Map([
