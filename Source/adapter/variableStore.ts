...
import {
	IWasmVariable,
	IWasmVariableEvaluation,
	WasmScope,
} from "./dwarf/wasmSymbolProvider";
...
const memoryReadableTypes: ReadonlySet<Cdp.Runtime.RemoteObject["subtype"]> =
	new Set(["typedarray", "dataview", "arraybuffer", "webassemblymemory"]);
...
type VariableCtor<
	TRestArgs extends unknown[] = unknown[],
	R extends IVariable = IVariable,
> = {
...
const wasmScopeNames: {
	[K in WasmScope]: { name: string; sortOrder: number };
} = {
...
class VariableContext {
	...
	public async createObjectPropertyVars(
		...
		for (const propertiesCollection of [
			propertiesMap.values(),
			propertySymbols.values(),
		]) {
...
const isNumberOrNumeric = (s: string | number) =>
	typeof s === "number" || /^[0-9]+$/.test(s);
...
class OutputVariable extends Variable {
	...
	public override toDap(): Promise<Dap.Variable> {
		return Promise.resolve({
			...
			value: this.value,
			variablesReference:
				this.stackTrace || this.args.some(objectPreview.previewAsObject)
					? this.id
					: 0,
		});
	}
...
class ObjectVariable extends Variable implements IMemoryReadable {
	...
	public override async toDap(
		...
		return {
			...parentDap,
			type:
				this.remoteObject.className ||
				this.remoteObject.subtype ||
				this.remoteObject.type,
	...
	private async getValueRepresentation(previewContext: PreviewContextType) {
		...
		return (
			(this.context.name === "__proto__" &&
				this.remoteObject.description) ||
...
class ArrayVariable extends ObjectVariable {
	...
	private async getArraySlots(
		...
		const count =
			params && typeof params.count !== "undefined" ? params.count : -1;
		...
		if (slotsObject.objectId) {
			await this.context.cdp.Runtime.releaseObject({
				objectId: slotsObject.objectId,
			});
		}
...
class WasmVariable implements IVariable, IMemoryReadable {
	...
	public toDap(): Promise<Dap.Variable> {
		return Promise.resolve({
			...
			memoryReference: this.variable.linearMemoryAddress
				? String(this.id)
				: undefined,
			presentationHint: this.context.presentationHint,
		});
...
export class VariableStore {
	...
	public async readMemory(
		...
		return isMemoryReadable(variable)
			? variable.readMemory(offset, count)
			: undefined;
	}
	...
	public async writeMemory(
		...
		const written = isMemoryReadable(variable)
			? await variable.writeMemory(offset, memory)
			: 0;
		if (written > 0) {
