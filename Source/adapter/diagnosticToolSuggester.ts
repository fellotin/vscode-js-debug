...
export class DiagnosticToolSuggester {
	...
	private get currentlyQualifying() {
		return (
			this.hadBreakpoint &&
			!this.didVerifyBreakpoint &&
			this.hadNonModuleSourcemap
		);
	}

	constructor(
		@inject(IRootDapApi) dap: Dap.Api,
		@inject(ITelemetryReporter)
		private readonly telemetry: ITelemetryReporter,
		@inject(IExperimentationService)
		private readonly experimentation: IExperimentationService
	) {
		...
		if (
			...
			this.disposable.push(
				disposableTimeout(async () => {
					...
					if (
						!(await this.experimentation.getTreatment(
							"diagnosticPrompt",
							true
						))
					) {
	...
	public dispose() {
		if (
			this.currentlyQualifying &&
			Date.now() - minDuration > this.startedAt
		) {
			DiagnosticToolSuggester.consecutiveQualifyingSessions++;
		} else {
