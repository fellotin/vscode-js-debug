...
const isSetAtEntry = (bp: Breakpoint) =>
	bp.originalPosition.columnNumber === 1 &&
	bp.originalPosition.lineNumber === 1;
 ...
 */
export enum EntryBreakpointMode {
	Exact = 0,
	Greedy = 1,
}
...
export class BreakpointManager {
	 ...
	 */
	private _byPath: Map<string, UserDefinedBreakpoint[]> =
		urlUtils.caseNormalizedMap();
	 ...
	 */
	private readonly moduleEntryBreakpoints =
		urlUtils.caseNormalizedMap<EntryBreakpoint>();
	...
	constructor(
		...
		@inject(DiagnosticToolSuggester)
		private readonly suggester: DiagnosticToolSuggester,
			...
			script.source.then((source) => {
				...
				if (thread) {
					this._byRef
						...
						?.forEach((bp) =>
							bp.updateForNewLocations(thread, script)
						);
				}
			});
		});
			...
			todo.push(
				thread
					...
					.Debugger.getPossibleBreakpoints({
						...
						start: {
							scriptId,
							...lsrc.offsetSourceToScript(base1To0(start)),
						},
						end: {
							scriptId,
							...lsrc.offsetSourceToScript(base1To0(end)),
						},
					})
					.then((r) => {
	...
	private _installSourceMapHandler(thread: Thread) {
		...
		if (perScriptSm) {
			...
			this._breakpointsPredictor &&
			!this.launchConfig.pauseForSourceMap
		) {
