...
import {
	StackFrameStepOverReason,
	shouldStepOverStackFrame,
} from "./smartStepping";
import {
	ISourceWithMap,
...
export class StackTrace {
		...
		while (this.frames.length < limit && this._asyncStackTraceId) {
			...
			if (noFuncEval) {
				this._lastFrameThread
					.cdp()
					.DotnetDebugger.setEvaluationOptions({
						options: { noFuncEval },
						type: "stackFrame",
					});
			}
	...
	private expandWasmFrames() {
		return (this._lastInlineWasmExpanded =
			this._lastInlineWasmExpanded.then(async (last) => {
...
export class StackFrame implements IStackFrameElement {
	...
	public equivalentTo(other: unknown) {
		return (
			...
			other._rawLocation.columnNumber ===
				this._rawLocation.columnNumber &&
	...
	async toDap(format?: Dap.StackFrameFormat): Promise<Dap.StackFrame> {
		const { isSmartStepped, name, uiLocation } =
		...
		return {
			...
			canRestart:
				(this.callFrame as Cdp.Debugger.CallFrame).canBeRestarted ??
				!this.isAsync,
		} as Dap.StackFrame;
	}
