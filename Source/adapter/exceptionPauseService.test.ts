...
describe("ExceptionPauseService", () => {
	...
	it("prints an error on conditional breakpoint parse error", async () => {
		...
		await ep.setBreakpoints({
			filters: [],
			filterOptions: [
				{ filterId: PauseOnExceptionsState.All, condition: "(" },
			],
		});
		expect(stubDap.output.args).to.containSubset([
			[{ category: "stderr" }],
		]);
	...
	it("prepares an expression if a condition is given", async () => {
		...
		await ep.setBreakpoints({
			...
			filterOptions: [
				{
					filterId: PauseOnExceptionsState.All,
					condition: 'error.name == "hi"',
				},
			],
		});
			...
			await ep.shouldPauseAt({
				callFrames: [
					upcastPartial<Cdp.Debugger.CallFrame>({
						callFrameId: "1",
						location: upcastPartial<Cdp.Debugger.Location>({
							scriptId: "42",
						}),
					}),
				],
