...
export class DebugAdapter implements IDisposable {
	...
	constructor(
		...
		this.dap.on("enableCustomBreakpoints", (params) =>
			this.enableCustomBreakpoints(params)
		);
		this.dap.on("toggleSkipFileStatus", (params) =>
			this._toggleSkipFileStatus(params)
		);
		this.dap.on("disableCustomBreakpoints", (params) =>
			this._disableCustomBreakpoints(params)
		);
	...
	private _breakpointLocations(
		...
		return this._withThread(async (thread) => {
			...
			const possibleBps =
				await this.breakpointManager.getBreakpointLocations(
					thread,
					source,
	...
	createThread(cdp: Cdp.Api, target: ITarget): Thread {
		...
		if (
			...
			profile.start(this.dap, this._thread, {
				type: BasicCpuProfiler.type,
			});
		}
	...
	async _prettyPrintSource(
		...
		await this.breakpointManager.moveBreakpoints(
			this._thread,
			source,
			sourceMap,
			generated
	...
	private onEvaluate(args: Dap.EvaluateParams): Promise<Dap.EvaluateResult> {
		...
		if (args.expression === ".scripts") {
			return this._dumpDiagnostics({ fromSuggestion: false })
				...
				.then(() => ({
					result: "Opening diagnostic tool...",
