...
import {
	ISourcePathResolver,
	InlineScriptOffset,
} from "../common/sourcePathResolver";
...
const isHMR = (url: string) =>
	url.endsWith(".hot-update.js") || viteHMRPattern.test(url);
...
export class SourceContainer {
	 ...
	 */
	private readonly scriptsById: Map<
	 ...
	 */
	public readonly onSourceMappedSteppingChange =
		this.onSourceMappedSteppingChangeEmitter.event;

	constructor(
		@inject(IDapApi) dap: Dap.Api,
		@inject(ISourceMapFactory)
		...
		@inject(IInitializeParams)
		private readonly initializeConfig: Dap.InitializeParams,
		...
		@inject(IResourceProvider)
		private readonly resourceProvider: IResourceProvider,
		@inject(IWasmSymbolProvider)
		private readonly wasmSymbols: IWasmSymbolProvider
	) {
		this._dap = dap;
		...
		if (mainRootPath) {
			...
			this.rootPaths = [
				mainRootPath,
				properResolve(mainRootPath, "..", "ClientApp"),
			];
		}
	...
	public async getOptiminalOriginalPosition(
		...
		const value =
			this.sourceMapFactory.guardSourceMapFn<NullableMappedPosition>(
				sourceMap,
				() => {
	...
	public async addSource(
		...
		this.logger.verbose(
			LogTag.RuntimeSourceCreate,
			"Creating source from url",
			{
				inputUrl: event.url,
				absolutePath,
			},
	...
	private async _finishAddSourceWithWasm(compiled: WasmSource) {
		...
		for (const url of symbols.files) {
			const absolutePath =
				await this.sourcePathResolver.urlToAbsolutePath({ url });
			...
			this.logger.verbose(
				LogTag.RuntimeSourceCreate,
				"Creating wasm source from source map",
				{
					inputUrl: url,
					absolutePath,
					resolvedUrl,
	...
	private async _finishAddSourceWithSourceMap(
		...
		this.logger.verbose(
			LogTag.SourceMapParsing,
			"Creating sources from source map",
			{
	...
	async _addSourceMapSources(compiled: ISourceWithMap, map: SourceMap) {
		...
		for (const url of map.sources) {
			...
			this.logger.verbose(
				LogTag.RuntimeSourceCreate,
				"Creating source from source map",
				{
					inputUrl: url,
					sourceMapId: map.id,
					absolutePath,
