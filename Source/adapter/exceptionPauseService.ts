...
export class ExceptionPauseService implements IExceptionPauseService {
	...
	public async shouldPauseAt(evt: Cdp.Debugger.PausedEvent) {
		...
		if (evt.data?.uncaught) {
			if (
				cond.uncaught &&
				!(await this.evalCondition(evt, cond.uncaught))
			) {
				return false;
			}
		} else if (cond.caught) {
	...
	protected parseBreakpointRequest(
		...
		if (cdp === PauseOnExceptionsState.None) {
			...
			return {
				cdp,
				condition: {
					caught: compile(caughtConditions),
					uncaught: compile(uncaughtConditions),
				},
			};
		}
	}
