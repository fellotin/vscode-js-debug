	...
	private async _onPaused(event: Cdp.Debugger.PausedEvent) {
		...
		const isSourceMapPause =
			...
			(event.reason === "instrumentation" ||
				...
				hitBreakpoints.some(
					(bp) => this._pauseOnSourceMapBreakpointIds?.includes(bp)
				));
	...
	async updateCustomBreakpoints(ids: CustomBreakpointId[]): Promise<void> {
		...
		for (const newId of newIds) {
			if (!this._enabledCustomBreakpoints.has(newId)) {
				todo.push(
					customBreakpoints().get(newId)?.apply(this._cdp, true)
				);
			}
		}
		for (const oldId of this._enabledCustomBreakpoints) {
			if (!newIds.has(oldId)) {
				todo.push(
					customBreakpoints().get(oldId)?.apply(this._cdp, false)
				);
			}
		}
			...
			hitBreakpointIds = await Promise.race([
				...
				Promise.all(
					details.hitBreakpoints
						.map((bp) =>
							this._breakpointManager._resolvedBreakpoints.get(bp)
						)
