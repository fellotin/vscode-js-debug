...
export class ExecutionContext {
	...
	constructor(
		public readonly description: Cdp.Runtime.ExecutionContextDescription,
	) {}
		...
		await Promise.all(
			this.scripts.map(async (s) => {
				...
				source.filterScripts(
					(s) => s.executionContextId !== this.description.id,
				);
...
export class Thread implements IVariableStoreLocationProvider {
	...
	static _allThreadsByDebuggerId = new Map<
	...
	public stepInto(targetId?: number): Promise<Dap.StepInResult | Dap.Error> {
		return this.stateQueue.enqueue("stepInto", async () => {
			...
			const stepInTarget =
				this._pausedDetails?.stepInTargets?.[targetId as number];
			if (stepInTarget) {
				const breakpoint = await this._cdp.Debugger.setBreakpoint({
					location: stepInTarget.breakLocation,
				});
				this._waitingForStepIn.intoTargetBreakpoint =
					breakpoint?.breakpointId;
				if (await this._cdp.Debugger.resume({})) {
					return {};
				}
			} else {
				const skipList = await this.getCurrentSkipList(
					StepDirection.In,
				);
				if (
	...
	_evaluateOnAsyncFrameError(): Dap.Error {
		return errors.createSilentError(
			l10n.t("Unable to evaluate on async stack frame"),
		);
	}

	async restartFrame(
		params: Dap.RestartFrameParams,
	): Promise<Dap.RestartFrameResult | Dap.Error> {
		const stackFrame = this._pausedDetails?.stackTrace.frame(
			params.frameId,
		)?.root;
		if (!stackFrame) {
		...
		if (!callFrameId) {
			return errors.createUserError(
				l10n.t("Cannot restart asynchronous frame"),
			);
		}
		...
		if (!ok) {
			return errors.createUserError(
				l10n.t("Frame could not be restarted"),
			);
		}
	...
	): Promise<Dap.ScopesResult | Dap.Error> {
	...
	public async completions(
		...
		const targets = await this.completer.completions({
			executionContextId: this._selectedContext
				? this._selectedContext.description.id
				: undefined,
			stackFrame,
		...
		return {
			targets: [
				...this.getExecutionContextCompletions(params),
				...targets,
			],
		};
	}

	private getExecutionContextCompletions(
		params: Dap.CompletionsParams,
	): Dap.CompletionItem[] {
	...
	private async _evaluteWasm(
		...
		const position = new Base0Position(
			stackFrame instanceof InlinedFrame
				? stackFrame.inlineFrameIndex
				: 0,
	...
	private async _evaluteJs(
		...
		const params: Cdp.Runtime.EvaluateParams =
			args.context === "clipboard"
				? {
						expression: serializeForClipboardTmpl.expr(
							args.expression,
							"2",
						),
				...
				: {
						...
						timeout:
							args.context === "hover"
								? this.getHoverEvalTimeout()
								: undefined,
				  };

		if (args.context === "repl") {
			params.expression = sourceUtils.wrapObjectLiteral(
				params.expression,
			);
			if (params.expression.indexOf("await") !== -1) {
				const rewritten = sourceUtils.rewriteTopLevelAwait(
					params.expression,
				);
				if (rewritten) {
		...
		const responsePromise = this.evaluator.evaluate(
			callFrameId
				...
				: {
						...params,
						contextId: this._selectedContext
							? this._selectedContext.description.id
							: undefined,
				  },
	...
	public async evaluate(
		...
		if (args.context === "repl" && args.expression.startsWith("cd ")) {
			...
			for (const ec of this._executionContexts.values()) {
				if (
					this.target.executionContextName(ec.description) ===
					contextName
				) {
		...
		const variableStore = callFrameId
			? this._pausedVariables
			: this.replVariables;
		if (!variableStore) {
			throw new ProtocolError(
				errors.createSilentError(l10n.t("Unable to evaluate")),
	...
	async _evaluateRepl(
		...
		if (response.exceptionDetails) {
			...
			throw new ProtocolError(
				errors.replError(formattedException.output),
			);
		}

		const contextName =
			this._selectedContext &&
			this.defaultExecutionContext() !== this._selectedContext
				? `\x1b[33m[${this.target.executionContextName(
						this._selectedContext.description,
				  )}] `
				: "";
		...
		if (
			resultVar.variablesReference === 0 &&
			resultVar.value.length === budget
		) {
			setImmediate(() =>
	...
	private _initialize() {
		...
		if (this.launchConfig.outputCapture === OutputSource.Console) {
			...
			this._cdp.Runtime.on("exceptionThrown", (event) => {
				this.console.enqueue(
					this,
					new ExceptionMessage(event.exceptionDetails),
				);
			});
		}
		...
		this._cdp.Debugger.on("scriptParsed", (event) =>
			this._onScriptParsed(event),
		);
		this._cdp.Debugger.on("scriptFailedToParse", (event) =>
			this._onScriptParsed(event),
		);
		...
		if (this.launchConfig.noDebug) {
			this.logger.info(
				...
				"Running with noDebug, so debug domains are disabled",
			);
		} else {
			this._ensureDebuggerEnabledAndRefreshDebuggerId();
		}
	...
	> {
		...
		const frame = pausedDetails.stackTrace.frames.find(
			(f) => f.frameId === frameId,
		);
		...
		const [locations, content] = await Promise.all([
			this._breakpointManager
				...
				.then((l) =>
					...
					l.filter(
						(l) =>
							l.breakLocation.lineNumber !==
								rawPausedLocation.lineNumber ||
							l.breakLocation.columnNumber !==
								rawPausedLocation.columnNumber,
					),
				),
		...
		pausedDetails.stepInTargets =
			pausedDetails.stepInTargets?.concat(locations) || locations;

		const lines = content && new PositionToOffset(content);

		return locations
			.map((location, i) => {
				const preferred = location.uiLocations.find(
					(l) => l.source === pausedLocation.source,
				);
				if (!preferred) {
					return;
				}
	...
	async refreshStackTrace() {
		...
		this._pausedDetails = await this._createPausedDetails(
	...
	private async _onPaused(event: Cdp.Debugger.PausedEvent) {
		...
		const isInspectBrk =
			(event.reason as string) === "Break on start" ||
			event.reason === "debugCommand";
		const location = event.callFrames[0]?.location as
			| Cdp.Debugger.Location
			| undefined;
		const scriptId =
			(event.data as IInstrumentationPauseAuxData)?.scriptId ||
			location?.scriptId;
		const isSourceMapPause =
			scriptId &&
			(event.reason === "instrumentation" ||
				this._breakpointManager.isEntrypointBreak(
					hitBreakpoints,
					scriptId,
				) ||
				hitBreakpoints.some((bp) =>
					this._pauseOnSourceMapBreakpointIds?.includes(bp),
				));
		this.evaluator.setReturnedValue(event.callFrames[0]?.returnValue);

		if (isSourceMapPause) {
			if (
				(this.launchConfig as IChromiumBaseConfiguration)
			...
			if (
				scriptId &&
				(await this._handleSourceMapPause(scriptId, location))
			) {
		...
		if (stepInBreakpoint) {
			if (event.hitBreakpoints?.includes(stepInBreakpoint)) {
				...
				await this._cdp.Debugger.removeBreakpoint({
					breakpointId: stepInBreakpoint,
				});
		...
		const pausedDetails = (this._pausedDetails =
			await this._createPausedDetails(event));
		if (this._excludedCallers.length) {
			if (
				await this._matchesExcludedCaller(
					this._pausedDetails.stackTrace,
				)
			) {
				this.logger.info(
					LogTag.Runtime,
					"Skipping pause due to excluded caller",
				);
				this.resume();
				return;
			}
		...
		const smartStepDirection =
			await this._smartStepper.getSmartStepDirection(
				pausedDetails,
				this._expectedPauseReason,
			);
	...
	private async _matchesExcludedCaller(trace: StackTrace): Promise<boolean> {
		...
		const first =
			firstFrame instanceof StackFrame && (await firstFrame.uiLocation());
		if (!first) {
			return false;
		}
		...
		for (const { caller, target } of this._excludedCallers) {
			if (
				target.line !== first.lineNumber ||
				target.column !== first.columnNumber
			) {
				continue;
			}
			...
			for (let i = 0; i < stackLocations.length; i++) {
				...
				if (
					!r ||
					r.lineNumber !== caller.line ||
					r.columnNumber !== caller.column
				) {
					continue;
				}

				const source = (stackAsDap[i] ??=
					await r.source.toDapShallow());
	...
	public rawLocationToUiLocation(
		...
		const script = this._sourceContainer.getScriptById(
	...
	private async _createPausedDetails(
		event: Cdp.Debugger.PausedEvent,
	): Promise<IPausedDetails> {
		...
		switch (event.reason) {
			...
			case "EventListener":
				return this._resolveEventListenerBreakpointDetails(
					stackTrace,
					event,
				);
			case "exception":
				...
				if (hitBreakpoints.length) {
					...
					if (
						...
						if (entryBreakpointSource !== undefined) {
							...
							isStopOnEntry = !entryBreakpointLocations?.some(
								(location) =>
									this._breakpointManager.hasAtLocation(
										location,
									),
							);
						}
					...
					if (!isStopOnEntry) {
						this._breakpointManager.registerBreakpointsHit(
							hitBreakpoints,
						);
					}
					return {
	...
	private _onScriptParsed(event: Cdp.Debugger.ScriptParsedEvent) {
		...
		let executionContext = this._executionContexts.get(
			event.executionContextId,
		);
		if (!executionContext) {
			this.logger.info(
				LogTag.Internal,
				"Creating missing execution context id",
				event,
			);
		...
		const createSource = async () => {
			const prevSource = this._sourceContainer.getSourceByOriginalUrl(
				event.url,
			);
			...
			const contentGetter = async () => {
				const response = await this._cdp.Debugger.getScriptSource({
					scriptId: event.scriptId,
				});
			...
			const inlineSourceOffset =
				event.startLine || event.startColumn
					? {
							lineOffset: event.startLine,
							columnOffset: event.startColumn,
					  }
					: undefined;
			...
			if (event.sourceMapURL) {
				...
				if (!resolvedSourceMapUrl) {
					this._dap.with((dap) =>
						errors.reportToConsole(
							dap,
							`Could not load source map from ${event.sourceMapURL}`,
						),
					);
				}
			}
			...
			const source = await this._sourceContainer.addSource(
				...
				!event.hasSourceURL && this.launchConfig.enableContentValidation
					? event.hash
					: undefined,
			);
	...
	async _handleSourceMapPause(
		...
		const perScriptTimeout =
			this._sourceContainer.sourceMapTimeouts().sourceMapMinPause;
		const timeout =
			perScriptTimeout +
		...
		return !!result
			...
			.some(
				(b) =>
					b.lineNumber === bLine &&
					(bColumn === undefined || bColumn === b.columnNumber),
			);
	}
	...
	private _getOrStartLoadingSourceMaps(script: Script) {
		...
		const result = script.source
			.then((source) =>
				this._sourceContainer.waitForSourceMapSources(source),
			)
			.then((sources) =>
	...
	async _copyObjectToClipboard(object: Cdp.Runtime.RemoteObject) {
		if (!object.objectId) {
			this._dap.with((dap) =>
				dap.copyRequested({
					text: objectPreview.previewRemoteObject(object, "copy"),
				}),
			);
			return;
		}
			...
			hitBreakpointIds = await Promise.race([
				...
				Promise.all(
					details.hitBreakpoints
						.map((bp) =>
							this._breakpointManager._resolvedBreakpoints.get(
								bp,
							),
						)
	...
	private isCrossThreadStep(event: Cdp.Debugger.PausedEvent) {
		...
		return parent._cdp.Debugger.getStackTrace({
			...
			const parentFrame = StackFrame.fromRuntime(
				parent,
				trace.stackTrace.callFrames[0],
				false,
			);
	...
	public async setScriptSourceMapHandler(
		...
		if (needsPause && !this._pauseOnSourceMapBreakpointIds?.length) {
			...
			this._pauseOnSourceMapBreakpointIds = result
				.map((r) => r?.breakpointId)
				.filter(truthy);
			...
			await Promise.all(
				breakpointIds.map((breakpointId) =>
					this._cdp.Debugger.removeBreakpoint({ breakpointId }),
				),
			);
		}
	...
	private async _enablePerScriptSourcemaps() {
		await this._breakpointManager.updateEntryBreakpointMode(
			this,
			EntryBreakpointMode.Greedy,
		);
		await this.setScriptSourceMapHandler(
			false,
			this._scriptWithSourceMapHandler,
		);
	}
	...
	private _shouldEnablePerScriptSms(event: Cdp.Debugger.PausedEvent) {
		...
		if (
			event.reason !== "instrumentation" ||
			!urlUtils.isDataUri(event.data?.sourceMapURL)
		) {
			return false;
		}
		...
		for (const chunk of new StackTraceParser(trace)) {
			...
			const compiledSource =
				this._sourceContainer.getSourceByOriginalUrl(
					urlUtils.absolutePathToFileUrl(chunk.path),
