...
export class WasmSymbolProvider implements IWasmSymbolProvider, IDisposable {
	...
	public async loadWasmSymbols(
		...
		this.logger.info(LogTag.SourceMapParsing, "parsed files from wasm", {
			files: result,
		});
	...
	private async getWorker() {
		...
		this.worker = dwarf.spawn({
			getWasmGlobal: (index, stopId) =>
				this.loadWasmValue(`globals[${index}]`, stopId),
			getWasmLocal: (index, stopId) =>
				this.loadWasmValue(`locals[${index}]`, stopId),
			getWasmOp: (index, stopId) =>
				this.loadWasmValue(`stack[${index}]`, stopId),
...
class WasmSymbols extends DecompiledWasmSymbols {
	private readonly mappedLines = new Map<
		/* source URL */ string,
		Promise<Uint32Array>
	>();
	...
	public override async originalPositionFor(
		...
		const locations = await this.rpc.sendMessage(
	...
	public async getVariablesInScope(
		...
		const variables = await this.rpc.sendMessage(
		...
		return variables.map(
			(v): IWasmVariable => ({
				...
				evaluate: async () => {
					...
					return result
						? new WasmVariableEvaluation(result, this.rpc)
						: nullType;
				},
			})
		);
	...
	public async getStepSkipList(
		...
		switch (direction) {
			case StepDirection.Out: {
				...
				rawRanges = await this.rpc.sendMessage(
	...
	private getMappedLines(sourceURL: string) {
		...
		const value = (async () => {
			try {
				const lines = await this.rpc.sendMessage(
					"getMappedLines",
					this.moduleId,
					sourceURL
				);
