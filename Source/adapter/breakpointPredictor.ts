...
export class BreakpointPredictorCachedState<T> {
	...
	constructor(
		...
		if (launchConfig.__workspaceCachePath) {
			this.path = path.join(
				launchConfig.__workspaceCachePath,
				"bp-predict.json"
			);
		}
	}
...
export class TargetedBreakpointSearch extends BreakpointSearch {
	private readonly sourcePathToCompiled = new Map<
		string,
		Promise<MetadataMap>
	>();
...
export class BreakpointsPredictor implements IBreakpointsPredictor {
	private readonly predictedLocations = new Map<
		string,
		IWorkspaceLocation[]
	>();
	...
	public async predictBreakpoints(
		...
		const addSourceMapLocations = async (
			...
			const map = await this.sourceMapFactory.load({
				...metadata,
				sourceMapUrl,
			});
