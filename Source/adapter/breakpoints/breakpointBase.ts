...
export abstract class Breakpoint {
	...
	public async updateUiLocations(
		...
		this.updateExistingCdpRef(cdpId, (bp) => {
			...
			return {
				...bp,
				locations: resolvedLocations,
				uiLocations: inPreferredSource.length
					? inPreferredSource
					: locations,
			};
		});
	...
	protected async _setPredicted(thread: Thread): Promise<void> {
		...
		const workspaceLocations =
			this._manager._breakpointsPredictor.predictedResolvedLocations({
	...
	protected async _setByPath(
		...
		if (sourceByPath instanceof SourceFromMap) {
			const mappedInPlace = [
				...sourceByPath.compiledToSourceUrl.keys(),
			].some((s) => s.absolutePath === this.source.path);
