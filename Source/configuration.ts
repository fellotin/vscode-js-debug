...
export interface IExtensionHostBaseConfiguration
...
export interface IExtensionHostLaunchConfiguration
 ...
 */
export interface INodeLaunchConfiguration
	extends INodeBaseConfiguration,
		IConfigurationWithEnv {
	type: DebugType.Node;
	request: "launch";
...
export interface IChromiumLaunchConfiguration
	extends IChromiumBaseConfiguration {
	request: "launch";
...
export interface IChromiumAttachConfiguration
	extends IChromiumBaseConfiguration {
	request: "attach";
...
export function defaultSourceMapPathOverrides(webRoot: string): {
	[key: string]: string;
} {
	return {
 ...
 * workspaces set.
 */
export function removeOptionalWorkspaceFolderUsages<
...
>(config: T): T {
	...
	if ("resolveSourceMapLocations" in cast) {
		cast.resolveSourceMapLocations =
			cast.resolveSourceMapLocations?.filter((o) => !o.includes(token)) ??
			null;
	}
