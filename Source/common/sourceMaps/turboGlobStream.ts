...
export class TurboGlobStream<E> {
	...
	constructor(opts: ITurboGlobStreamOptions<E>) {
		...
		const pattern = isAbsolute(opts.pattern)
			? relative(opts.cwd, opts.pattern)
			: opts.pattern;
	...
	private getDirectoryReadDescends(
		...
		if (token === "**") {
			...
			if (dirent.type === CachedType.File) {
				return isTerminal
					? 0
					: this.getDirectoryReadDescends(ctx, ti + 1, path, dirent);
			}
		...
		if (
			(token instanceof RegExp && token.test(dirent.name)) ||
			token === dirent.name
		) {
			// directories cannot be the terminal match, and files must be
			if (
				dirent.type === CachedType.Directory ? !isTerminal : isTerminal
			) {
				return 1;
			}
		}
