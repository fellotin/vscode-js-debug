...
describe("urlUtils", () => {
	describe("fileUrlToPath()", () => {
		it("removes file:///", () => {
			expect(fileUrlToAbsolutePath("file:///c:/file.js")).to.equal(
				"c:\\file.js",
			);
		});
		...
		it("unescape when doing url -> path", () => {
			expect(
				fileUrlToAbsolutePath("file:///c:/path%20with%20spaces"),
			).to.equal("c:\\path with spaces");
		});

		it("ensures local path starts with / on OSX", () => {
			overridePlatform("darwin");
			expect(
				fileUrlToAbsolutePath("file:///Users/scripts/app.js"),
			).to.equal("/Users/scripts/app.js");
			resetPlatform();
		});

		it("force lowercase drive letter on Win to match VS Code", () => {
			// note default 'os' mock is win32
			expect(fileUrlToAbsolutePath("file:///D:/FILE.js")).to.equal(
				"d:\\FILE.js",
			);
		});
	...
	describe("urlToRegex - case sensitive", () => {
		...
		it("works with a pre-escaped prefix", () => {
			...
			expect(
				urlToRegex(url, [wildcardHostname.length, url.length]),
			).to.equal("https?:\\/\\/[^\\/]+\\/foo($|\\?)");
		});
	});
	...
	describe("createTargetFilter()", () => {
		...
		it("does not return substring fuzzy match as in pre 0.1.9", () => {
			testAll("http://localhost/site/page", [
				["http://localhost/site", false],
			]);
		});
	...
	describe("isLoopback", () => {
		...
		beforeEach(() => {
			...
			lookupStub
				.withArgs("local.contoso.com")
				.resolves({ address: "127.0.0.1" });
		});
