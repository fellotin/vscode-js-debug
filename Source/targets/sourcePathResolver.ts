...
export abstract class SourcePathResolverBase<
	T extends ISourcePathResolverOptions,
> implements ISourcePathResolver
{
	 ...
	 */
	private readonly resolvePatterns =
		this.options.resolveSourceMapLocations?.map((location) => {
			...
			suffix =
				suffix
					.slice(0, sharedPrefixLen)
					.replace(/[\[\]\(\)\{\}\!\*]/g, "\\$&") + // CodeQL [SM02383] backslashes are not present in this string
	...
	public shouldResolveSourceMap({
		...
		const l = match(
			testLocations.map(forceForwardSlashes),
