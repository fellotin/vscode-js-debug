	...
	private async applyInspectorOptions(
		...
		try {
			...
			if (
				...
				e.cause.id === ErrorCodes.CannotFindNodeBinary
			) {
		...
		variables.description = new vscode.MarkdownString(
			l10n.t({
				message:
					'Enables Node.js [auto attach]({0}) debugging in "{1}" mode',
				comment: [
					"{Locked='[auto attach]({0})'}",
					"the 2nd placeholder is the setting value",
				],
				args: [
	...
	protected async getBootloaderFile(
		...
		if (storagePath.includes(" ")) {
			if (
				...
				!binary.has(Capability.UseSpacesInRequirePath)
			) {
