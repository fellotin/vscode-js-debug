...
@injectable()
export abstract class NodeLauncherBase<T extends AnyNodeConfiguration>
	implements ILauncher
{
	...
	public async restart(newParams: AnyLaunchConfiguration): Promise<void> {
		...
		await this.launchProgram({
			...this.run,
			params: newParams
				? this.resolveParams(newParams) ?? this.run.params
				: this.run.params,
			context: {
				...this.run.context,
				cancellationToken:
	...
	protected async resolveEnvironment(
		...
		const bootloaderInfo: IBootloaderInfo = {
			...
			verbose:
				params.trace === true ||
				(typeof params.trace === "object" && params.trace.stdio)
					? true
					: undefined,
			autoAttachMode: AutoAttachMode.Always,
			mandatePortTracking: this.portLeaseTracker.isMandated
				? true
				: undefined,
			...additionalOptions,
		};
		...
		const env = {
			...
			VSCODE_INSPECTOR_OPTIONS:
				variableDelimiter + JSON.stringify(bootloaderInfo),
