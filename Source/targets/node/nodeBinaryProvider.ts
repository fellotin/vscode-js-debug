...
export enum Capability {
	UseSpacesInRequirePath = 0,
	UseInspectPublishUid = 1,
}
...
const packageManagers: ReadonlySet<string> = new Set([
	"npm",
	"yarn",
	"pnpm",
	"tnpm",
...
export const isPackageManager = (exe: string) =>
	packageManagers.has(basename(exe, extname(exe)));
...
export class NodeBinary {
	...
	public get warning() {
		...
		for (const message of warningMessages) {
			if (
				message.inclusiveMax.gte(this.version) &&
				message.inclusiveMin.lte(this.version)
			) {
				return message;
			}
		}
...
export class NodeBinaryProvider {
	...
	private async resolveAndValidateInner(
		...
		if (!nodeOrElectronBinaries.includes(exeName)) {
			...
			try {
				const realBinary = await this.resolveAndValidateInner(
					env,
					"node",
					undefined,
					cwd,
		...
		this.logger.info(LogTag.RuntimeLaunch, "Discovered version", {
			version: versionText,
		});
...
export class InteractiveNodeBinaryProvider extends NodeBinaryProvider {
	 ...
	 * @override
	 */
	protected async shouldTryDebuggingAnyway({
		message,
	}: NodeBinaryOutOfDateError) {
		if (!this.vscode) {
			return false;
		}
