...
describe("NodeBinaryProvider", function () {
	...
	describe("electron versioning", () => {
		...
		it("remaps to node version on electron with .cmd", async () => {
			if (process.platform === "win32") {
				getVersionText
					.withArgs("/foo/electron.cmd")
					.resolves("\nv6.1.2\n");
				getVersionText.withArgs("/node").resolves("v14.5.0");
				resolveBinaryLocation
					.withArgs("electron")
					.returns("/foo/electron.cmd");
	...
	const matrix = [
		{
			v: "10.0.0",
			c: {
				[Capability.UseInspectPublishUid]: false,
				[Capability.UseSpacesInRequirePath]: false,
			},
		},
		{
			v: "12.0.0",
			c: {
				[Capability.UseInspectPublishUid]: false,
				[Capability.UseSpacesInRequirePath]: true,
			},
		},
		{
			v: "12.8.0",
			c: {
				[Capability.UseInspectPublishUid]: true,
				[Capability.UseSpacesInRequirePath]: true,
			},
		},
	];
