...
import {
	...
	IOptions,
} from "../../targets/browser/browserPathResolver";
...
describe("BrowserPathResolver", () => {
		...
		describe("VueFileMapper", () => {
			...
			it("has correct vue handling", () => {
				const ttable = new Map([
					...
					[
						"webpack:///./components/hello.vue?f00d",
						VueHandling.Omit,
					],
			...
			it("maps basenames to disk", async () => {
				...
				expect(await mapper.lookup("webpack:///q.vue?f00d")).to.be
					.undefined;
				expect(await mapper.lookup("webpack:///unrelated.js")).to.be
					.undefined;
			});
		});
	});
	...
	[
		...
		it(`returns ${folder} for ${client} if the webroot path doesn't exist and the modified path does`, async () => {
			const webRoot =
				"c:\\Users\\user\\Source\\Repos\\Angular Project\\wwwroot";
			...
			const absolutePath = await resolver.urlToAbsolutePath({
				url,
				map: upcastPartial<SourceMap>({
					metadata: { sourceMapUrl: "", compiledPath: "x" },
				}),
			});
