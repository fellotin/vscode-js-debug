...
import {
	ISourcePathResolverOptions,
	SourcePathResolverBase,
...
export class BrowserSourcePathResolver extends SourcePathResolverBase<IOptions> {
	...
	private absolutePathToUrlPath(absolutePath: string): {
		...
		if (!bestMatch) {
			return {
				url: utils.absolutePathToFileUrl(absolutePath),
				needsWildcard: false,
			};
		}
		...
		return {
			url: utils.completeUrlEscapingRoot(baseUrl, urlPath),
			needsWildcard: false,
		};
	}

	public async urlToAbsolutePath({
		url,
		map,
	}: IUrlResolution): Promise<string | undefined> {
		const queryCharacter = url.indexOf("?");

		// Workaround for vue, see https://github.com/microsoft/vscode-js-debug/issues/239
		if (
			queryCharacter !== -1 &&
			url.slice(queryCharacter - 4, queryCharacter) !== ".vue"
		) {
			url = url.slice(0, queryCharacter);
		}

		return map
			? this.sourceMapSourceToAbsolute(url, map)
			: this.simpleUrlToAbsolute(url);
	}
	...
	private async simpleUrlToAbsolute(url: string) {
		...
		while (pathParts.length > 0) {
			...
			if (clientPath) {
				if (
					!extname &&
					(await this.fsUtils.exists(clientPath + Suffix.Html))
				) {
