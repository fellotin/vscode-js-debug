...
export interface IDapInitializeParamsWithExtensions
	extends Dap.InitializeParams {
	...
	implements ILauncher
{
	...
	constructor(
		...
		private readonly initializeParams: Dap.InitializeParams,
	...
	protected async launchBrowser(
		...
		return await launcher.launch(
			...
			{
				onStdout: (output) =>
					dap.output({ category: "stdout", output }),
				onStderr: (output) =>
					dap.output({ category: "stderr", output }),
	...
	private async prepareLaunch(
		...
		this._disposables.push(
			launched.cdp.onDisconnected(() => this.fireTerminatedEvent())
		);
